    ...
    switch(status){
        ...
        case SUSPECT:
            ...
            if ((prev_status == SUSPECT) && (missed_packets >= 3)){
                ...
                missed_packets++;
                status = TRANSMITTING; //Try again
                return 0; // \_('o')_/ Idk man, seems right
            }
...
void Module::FrameMessage(short data, uint8_t* message){
    ...
    if (this-> status == TRANSMITTING){
        ...
        //Compute the checksum value, append
        message[4] = this->Checksum(message, 4);
...
short Module::ParseMessage(const uint8_t* message){
    ...
    uint8_t data_payload[4]= {message[1], message[2], message[3], message[4]};
    bool correct_checksum_value = this->ValidChecksum(data_payload, message[5], 4);
...
bool Module::IsConnectionEstablished(const uint8_t* handshake){
    ...
    uint8_t data_payload[4] = {handshake[1], handshake[2], handshake[3], handshake[4]};
    bool checksum_valid = this -> ValidChecksum(data_payload, handshake[5], 4);
